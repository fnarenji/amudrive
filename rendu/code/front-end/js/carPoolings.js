...
myApp.controller('CarPoolingController', ['$scope', 'REST', 'mapService', 'sessionService', 'alertService', function($scope, REST, mapService, sessionService, alertService){
    ...
    mapService.setMarkerCallback(function (json) {
        ...
        $('#search-panel-name').html(json.firstname + ' ' + json.lastname + ' - ' + json.name);
        ...
        $('#search-panel-select-radio').css('background-image', 'url(img/radio_' + (json.radio ? 'on' : 'off') + '.png)');
        $('#search-panel-select-luggage').css('background-image', 'url(img/luggage_' + (json.luggage > 0 ? 'on' : 'off') + '.png)');
